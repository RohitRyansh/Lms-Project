...
<div class="allcontent">
    ...
    <div class="courses1">
        ...
        <form action=" {{ route ('tests.questions.update', ['course' => $course, 'unit' => $unit, 'test' => $test, 'question' => $question]) }} " method="post" class="CourseCreate" enctype="multipart/form-data">
            @csrf

            <label for="exampleFormControlInput1" class="form-label">Type of Question</label>
            <input type="text" name="question" class="form-control" id="exampleFormControlInput1" placeholder="Enter Test Name" value="{{ $question->question }}">
            <span class="errorMessage">
                @error('question')
                 {{ $message }}      
                @enderror
            ...
            <label for="exampleFormControlInput1" class="form-label">Options</label>
                <div class="btn-group">
                    ...
                    <input type="radio" name="answer" id="check" value="{{ $option->answer }}"/><input type="text" name="options[]" id="" value="{{ $option->option }}">
                    @endforeach
                </div>          
   
            <div class="saveButtons">
                <button type="submit" value="save" name="save" class="btn btn-secondary">Update</button>
